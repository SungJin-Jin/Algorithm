...
private fun solve(first: String, second: String): Int {
    ...
    val union = unionSize(firstUnits, secondUnits)
...
private fun unionSize(first: List<String>, second: List<String>): Int = when {
    first.size >= second.size -> first.plus(second.filter { !first.contains(it) }).size
    else -> second.plus(first.filter { !second.contains(it) }).size
}

private fun intersectionSize(first: List<String>, second: List<String>): Int = when {
    first.size >= second.size -> second.filter { first.contains(it) }.size
    else -> first.filter { second.contains(it) }.size
}
