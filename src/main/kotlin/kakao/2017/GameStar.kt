...
private fun solve(input: String): Int {
    ...
    while (index < input.length) {
        val score = createScore(index, input)
        if (score == 10) index += 2
        else index++

        val bonus = createBonus(input[index])
        index++

        val option = try {
            createOption(input[index])
        } catch(e: Exception) {
            1
        }
        if (option != 1) index++
...
private fun createScore(index: Int, input: String): Int {
    var score = input[index] - '0'
    if (score == 1) {
        val nextScore = input[index + 1] - '0'
        if (nextScore == 0) {
            score = 10
        }
    }
...
private fun createBonus(ch: Char): Int = when (ch) {
    'S' -> 1
    'D' -> 2
    'T' -> 3
    else -> 0
...
private fun createOption(ch: Char): Int = when (ch) {
    '#' -> -1
    '*' -> 2
    else -> 1
}
